...
def plot_learning_curve_from_csv(csv_path: Path):
    ...
    val_acc  = [float(r["val_accuracy"]) for r in rows] if "val_accuracy" in rows[0] else None
...
def main():
    ...
    parser.add_argument("--model", type=Path, default=Path("model.keras"),
                        help="Ścieżka do pliku modelu .keras (domyślnie model.keras).")
    parser.add_argument("--curve-csv", type=Path, default=Path("learning_curve.csv"),
